...
## 2. Rychlé spuštění celé pipeline (volitelné)
Pokud chcete odbavit všechny kroky jedním příkazem, použijte připravený skript:
```bash
cd nextjs-app
NEXT_PUBLIC_SUPABASE_URL="https://..." \
SUPABASE_SERVICE_ROLE_KEY="eyJ..." \
SUPABASE_DB_URL="postgresql://..." \
SUPABASE_PROJECT_REF="wsgm..." \
npm run deploy:all
```
Za sebou se spustí nasazení schématu, seed testovacích účtů i deploy Edge
Functions. Chcete-li mít nad jednotlivými kroky větší kontrolu, postupujte
podle následujících sekcí.

## 3. Nasazení databázového schématu
```bash
cd nextjs-app
...
## 6. Rychlá kontrola PWA funkčnosti
