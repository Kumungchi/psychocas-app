...
function LoginContent() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!cleanPhone) {
        setMessage({ type: 'error', translationKey: 'login.messages.errorInvalidPhone' });
        setIsLoading(false);
        return;
      }
      ...
      // Use phone authentication with Supabase
      ...
      if (error) {
        ...
        if (status === 429) {
          ...
          setMessage({
            type: 'error',
            text: 'Phone number not authorized. Contact admin to be added to the system.'
          });
        } else if (hasMessage) {
        ...
        <div className="psychocas-card auth-card text-center">
          ...
          <div className="mb-10 space-y-3 sm:mb-12">
            ...
            <p className="auth-card__subtitle">
              {otpSent ? t('login.subtitleOtpSent') : t('login.subtitlePhone')}
            </p>
          </div>
          ...
          {/* Phone Number Form - Show when OTP not sent yet */}
          {!otpSent && (
            <form
